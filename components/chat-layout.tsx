...
export default function ChatLayout({ children }: ChatLayoutProps) {
  ...
  return (
    <div className="flex h-screen bg-background">
      ...
      {/* Left Sidebar Mobile */}
      <div
        className={`fixed left-0 top-0 h-screen w-64 bg-card border-r border-border z-40 transform transition-transform duration-300 md:hidden ${
      ...
      <div
        className={`fixed left-0 top-0 h-screen w-72 bg-card border-r border-border z-40 transform transition-transform duration-300 md:hidden ${
          mobileMenuOpen ? "translate-x-64" : "-translate-x-full"
        }`}
      >
        <GroupsSidebar />
      </div>
